...
static SHOW_COMP_OPTION plugin_status(THD *ctl_thd, const LEX_CSTRING *name,
                                      int type)
{
  ...
  if (ctl_thd)
  {
    mysql_mutex_lock(&ctl_thd->LOCK_thd_data);
    st= ctl_thd->plugins_state;
  }
  else
  ...
  if (ctl_thd)
    mysql_mutex_unlock(&ctl_thd->LOCK_thd_data);
  DBUG_RETURN(rc);
}


bool plugin_is_ready(THD *ctl_thd, const LEX_CSTRING *name, int type)
{
  return plugin_status(ctl_thd, name, type) == SHOW_OPTION_YES;
}
...
/*
  ...
  unlock happens later.
*/
plugin_ref plugin_lock_by_name(THD *ctl_thd, THD *thd,
...
bool plugin_foreach_with_mask(THD *ctl_thd, THD *thd, plugin_foreach_func *func,
                       int type, uint state_mask, void *arg)
{
  ...
  */
  if (type == MYSQL_ANY_PLUGIN)
  {
    plugins= (plugin_ref*) my_alloca(st->m_array.elements * sizeof(plugin_ref));
